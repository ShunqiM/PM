...
from experiments.API.API_LLaVA.functions import getmask, get_model
from experiments.API.API_LLaVA.hook import hook_logger
from experiments.API.DatasetManager.DatasetManager.dataloader import get_data
...
def blend_mask_main(image_path_or_pil_image, mask, key, enhance_coe, kernel_size, interpolate_method, folder):
    mask = revise_mask(mask.float(), kernel_size = kernel_size, enhance_coe = enhance_coe)
    mask = mask.detach().cpu()
    mask = toImg(mask.reshape(1,24,24))
