...
from experiments.API.API_CLIP.clip_prs.utils.factory import create_model_and_transforms, get_tokenizer
from experiments.API.API_CLIP.hook import hook_prs_logger
from experiments.API.DatasetManager.DatasetManager.dataloader import get_data, Disposable_Dataloader
...
def modify_clip(model, layer_index, device = "cuda:0"):
    pass
...
def gen_mask_main(model, prs, preprocess, device, tokenizer, image_path_or_pil_images, questions):
    ## Load image
    images = []
    image_pils = []
    for image_path_or_pil_image in image_path_or_pil_images:
...
def blend_mask(image, cls_mask, patch_mask, enhance_coe, kernel_size, interpolate_method_name, grayscale):
    ...
    array = mask.cpu().detach().numpy()
    import matplotlib.pyplot as plt
    plt.figure(figsize=(8, 6))  # Optional: set figure size
    plt.imshow(array, cmap='hot', interpolation='nearest')
    plt.colorbar()  # Optional: add color bar
    ...
    mask.convert("L").convert("RGB").show()
