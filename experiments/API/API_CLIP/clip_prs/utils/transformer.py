...
class MultiheadAttention(nn.Module):
    ...
    def forward_per_head(self, x, attn_mask=None):
        ...
        x = torch.einsum('bhnm,bhmc->bnmhc', attn, v) # We also switch here back from head-first to n-first
        x = self.hook('extended_attn_v', ret=x)
        # print('zzz attention out post', torch.einsum('bnmhc,dhc->bnmd', x, self.out_proj.weight.reshape(self.embed_dim, self.num_heads, self.head_dim)).shape)
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # print('x shape in res block post', x.shape)
